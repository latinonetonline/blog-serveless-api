const articleSlugs = [
    'introducción-a-blazor',
    'los-secretos-del-monitoreo-de-sql-server',
    '.net-core-ef-core-sql-server-angular-azure',
    'improving-our-ui-skills-in-xamarin-forms',
    'mejor-que-github-pages-azure-static-web-apps',
    'azure-functions-en-la-vida-real',
    'desarrollo-multiplataforma-con-flutter-y-.net-core',
    'visual-studio-codespaces-edición-de-código-online',
    'xamarin-bindings-para-librerías-de-androidios',
    'docker-virtualización-para-niños',
    'azure-devops-pipelines,-integraciones-y-cicd',
    'no-te-pierdas-ningun-minuto-de-tu-app-con-signalr',
    'gestión-de-documentos-office-con-open-xml-sdk',
    'logging-efectivo-con-serilog-elasticsearch-kibana',
    'autenticación-faceid-y-geolocalizacion-en-ios-con-xamarin',
    'unit-testing-consejos-para-escribir-tests-envidiables',
    'c-8-&-.net-5-primeros-pasos',
    'integrar-ml.net-a-nuestras-aplicaciones-usando-model-builder',
    'dale-dinamismo-a-tu-código-c',
    'nos-reservamos-el-derecho-de-admisión-oauth-.net-core'
]

module.exports = articleSlugs
